...
const assertDisplayGroupStats = (expected) => assert.equal(JSON.stringify(state.displayGroupCommunityStatsById), JSON.stringify(expected));
...
assert.equal(calls, 0); assertDisplayGroupStats(initial()); assert.equal(modalRefreshes, 0);
...
assert.equal(calls, 1); assertDisplayGroupStats(initial()); assert.equal(modalRefreshes, 0);
...
state.displayGroupCommunityStatsById = initial(); modalRefreshes = 0; next = deferred(); const failed = context.fetchDisplayGroupCommunityStats(1); assertDisplayGroupStats(initial()); next.resolve({ data: null, error: new Error("fail") }); await failed;
assertDisplayGroupStats({}); assert.deepEqual(state.communityStatsByKey, { ordinary: { averageRating: 8, ratingCount: 2 } }); assert.equal(modalRefreshes, 1);
state.displayGroupCommunityStatsById = initial(); ids = []; modalRefreshes = 0; await context.fetchDisplayGroupCommunityStats(1); assertDisplayGroupStats({}); assert.equal(modalRefreshes, 1); ids = ["narkobetjenten"];
...
assertDisplayGroupStats(initial()); assert.equal(modalRefreshes, 0);
next = deferred(); const staleToken = context.fetchDisplayGroupCommunityStats(1); state.communityStatsRequestToken = 2; next.resolve({ data: null, error: new Error("old") }); await staleToken; assertDisplayGroupStats(initial()); assert.equal(modalRefreshes, 0);
